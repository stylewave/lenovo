		...
		<form action="<?php echo U('insert');?>" method="post">
			...
			<div class="form-group">
				...
				<select name="cid" class="form-control" id="">
					<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class): $mod = ($i % 2 );++$i; $arr=explode(',',$class[path]); $size=count($arr)-2; ?>
						<?php if($size == 2): ?><option value="<?php echo ($class[id]); ?>"><?php echo str_repeat('|====',$size); echo ($class[cname]); ?></option>
						<?php else: ?>
							<option disabled value="<?php echo ($class[id]); ?>"><?php echo str_repeat('|====',$size); echo ($class[cname]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				</select>
			...
			<div class="form-group">
				...
				<select name="bid" class="form-control" id="">
					<?php if(is_array($brand)): $i = 0; $__LIST__ = $brand;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class): $mod = ($i % 2 );++$i;?><option value="<?php echo ($class[id]); ?>"><?php echo ($class[bname]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
...
// 图片上传
	...
	$('#ups').uploadify({
    	...
    	'uploader' : '<?php echo U("Goods/ajax_up");?>',   //上传的方法
    	'buttonText' : '商品图片上传',
    	onUploadSuccess : function(file, data, response) {
			//把所有上传的图片都放入DIV中
			$("#img_post").val(data);
			img = "<img width='200px' src='/Uploads/Goods/"+data+"'>";
			$('#imgs').html(img);
		}
	});
